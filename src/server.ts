...
export function createApp() {
  ...
  const deepHealthEnabled = !isProduction || process.env.ENABLE_PUBLIC_DEEP_HEALTHCHECK === 'true';
  ...
  app.get('/health/deep', deepHealthRateLimiter, async (c) => {
    if (!deepHealthEnabled) {
      return c.json({
        error: 'Not found',
        code: 'NOT_FOUND',
        message: 'Deep health checks are disabled in production by default.',
      }, 404);
    }
  ...
  app.route('/v3/documents', documentsRoutes);
  app.route('/v1/search', searchRoutes);
  app.route('/v4/search', searchRoutes);
